...
def test_backend_failure_identifies_pass_and_retains_cause():
    from writing_assistant.passes import TONE
    error = RuntimeError("synthetic backend failure")
    class BrokenBackend:
        def generate(self, prompt):
            raise error
    with pytest.raises(RuntimeError, match="pass 'tone' failed") as failure:
        Pipeline([TONE], BrokenBackend()).run("Synthetic draft")
    assert failure.value.__cause__ is error


class TestExecutableAndCritiquePasses:
    def test_executable_passes_chain_with_builtin_and_keep_provenance(self):
        from writing_assistant.style import DesiredStyleProfile
        from writing_assistant.types import Pass
        profile = DesiredStyleProfile(tone="friendly")
        backend = MockLLM(["Model revision."])
        calls = []

        def transform(text, received_profile, received_backend):
            assert received_profile is profile
            assert received_backend is backend
            calls.append(text)
            return text + " Added."

        custom = Pass("append", "", executor=transform)
        results = Pipeline([custom, CLARITY, custom], backend, profile).run("Synthetic.")
        assert calls == ["Synthetic.", "Model revision."]
        assert [r.pass_name for r in results] == ["append", "clarity", "append"]
        assert results[1].original == "Synthetic. Added."
        assert results[-1].revised == "Model revision. Added."
        assert all(r.diff for r in results)

    def test_executor_does_not_implicitly_call_backend_and_can_delete_text(self):
        from writing_assistant.types import Pass
        class UnusedBackend:
            def generate(self, prompt):
                raise AssertionError("backend must not run")
        result = Pipeline([Pass("delete", "", executor=lambda *_: "")], UnusedBackend()).run("Delete.")[0]
        assert result.revised == ""
        assert "-Delete." in result.diff

    def test_executor_failure_preserves_cause_and_pass_identity(self):
        from writing_assistant.types import Pass
        def fail(*args):
            raise ValueError("synthetic callback failure")
        with pytest.raises(RuntimeError, match="'broken'.*synthetic callback failure") as error:
            Pipeline([Pass("broken", "", executor=fail)], MockLLM()).run("Text.")
        assert isinstance(error.value.__cause__, ValueError)

    def test_executor_rejects_non_string_before_next_pass(self):
        from writing_assistant.types import Pass
        reached = []
        with pytest.raises(RuntimeError, match="'invalid'.*output must be a string"):
            Pipeline([
                Pass("invalid", "", executor=lambda *_: None),
                Pass("later", "", executor=lambda *_: reached.append(True)),
            ], MockLLM()).run("Text.")
        assert reached == []
        with pytest.raises(TypeError, match="executor must be callable"):
            Pass("invalid", "", executor=3)

    def test_critique_receives_original_revisions_and_exact_diffs(self):
        from writing_assistant.passes import CRITIQUE
        prompts = []
        class Backend:
            def generate(self, prompt):
                prompts.append(prompt)
                return "Revised synthetic.\n" if len(prompts) == 1 else "  Specific critique.\n"
        results = Pipeline([CLARITY, CRITIQUE], Backend()).run("Original synthetic.\n")
        assert len(prompts) == 2
        assert "Original synthetic.\n" in prompts[1]
        assert "Revised synthetic.\n" in prompts[1]
        assert results[0].diff in prompts[1]
        assert "Do not rewrite" in prompts[1]
        assert results[-1].revised == "  Specific critique.\n"
        assert results[-1].pass_name == "critique"

    def test_nonterminal_critique_rejected_before_any_execution(self):
        from writing_assistant.passes import CRITIQUE
        class Backend:
            def generate(self, prompt):
                raise AssertionError("no backend execution expected")
        with pytest.raises(ValueError, match="final pass"):
            Pipeline([CRITIQUE, CLARITY], Backend()).run("Synthetic.")
